...
st.set_page_config(
    page_title="Brad ou Bad ? 🎭 - Agent IA",
    page_icon="🎭",
    layout="centered",
...
def display_header():
    ...
    st.title("🎭 Brad ou Bad ? - Agent IA")
    st.subheader("Coach IA anti-brouteur avec Intelligence Artificielle")
    ...
    stage_descriptions = {
        ...
        2: "🟠 Informations Personnelles - Brad demande des détails privés",
        3: "🔴 Manipulation Émotionnelle - Brad utilise l'émotion",
        4: "🚨 Demande d'Argent - Brad révèle ses vraies intentions",
        5: "💀 Chantage & Escalade - Brad devient agressif"
    }
    
    st.markdown(f"""
    <div style="background: rgba(134, 94, 247, 0.1); padding: 15px; border-radius: 10px; margin: 20px 0;">
...
def display_conversation():
    ...
    if not agent.conversation_history:
        st.markdown("""
        <div style="text-align: center; padding: 40px; background: rgba(255, 107, 107, 0.1); border-radius: 15px; margin: 20px 0;">
    ...
    for i, exchange in enumerate(agent.conversation_history):
        ...
        if st.session_state.show_analysis and 'analysis' in exchange:
            ...
            with st.expander(f"🔍 Analyse IA - Échange #{i+1}", expanded=False):
...
def handle_user_response(user_input):
    """Traite la réponse de l'utilisateur avec l'agent IA"""
    ...
    with st.spinner("🤖 Brad réfléchit à sa réponse..."):
        try:
            ...
            st.success("✅ Nouvelle réponse de Brad générée !")
...
def display_game_interface():
    """Affiche l'interface principale du jeu avec l'agent IA"""
    ...
    if not st.session_state.conversation_ended:
        ...
        with st.form("ai_response_form", clear_on_submit=True):
            user_input = st.text_area(
                "Comment répondez-vous à Brad ?",
                placeholder="Tapez votre réponse naturelle ici... L'IA s'adaptera à votre style !",
                height=120,
                help="💡 Répondez naturellement ! L'agent IA Brad va analyser votre réponse et s'adapter en conséquence."
            )
...
def display_final_results():
    """Affiche les résultats finaux avec l'évaluation IA"""
    agent = st.session_state.brad_agent
    ...
    with col2:
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #845ef7, #7048e8); color: white; padding: 30px; border-radius: 15px; text-align: center; margin: 20px 0;">
    ...
    with col1:
        ...
        if vulnerability_profile['detected_weaknesses']:
            st.markdown("**⚠️ Faiblesses détectées par l'IA:**")
    ...
    if agent.conversation_history:
        ...
        if trust_evolution:
            ...
            fig.update_layout(
                title="Évolution pendant la conversation",
    ...
    with col3:
        if st.button("📱 Partager résultats", use_container_width=True):
            ...
            st.success(f"🎯 J'ai testé ma résistance aux arnaques avec Brad ou Bad et j'ai obtenu {score}/20 ! 🛡️")

def main():
    """Fonction principale de l'application IA"""
    ...
    with st.sidebar:
        ...
        st.session_state.show_analysis = st.checkbox("🔍 Afficher l'analyse IA", value=st.session_state.show_analysis)
        
        st.markdown("### 📝 À propos")
        st.info("""
        **Brad ou Bad ?** utilise l'intelligence artificielle pour créer un faux Brad Pitt qui s'adapte à vos réponses.
        
        L'IA analyse votre psychologie en temps réel et ajuste sa stratégie d'arnaque pour tester votre vigilance !
        """)
